...
# save Fig_1_Results
write.csv(Fig_1_Results, "Tabs/Fig_1_Results.csv")
  
  # Add a flag for shading
  graph_1 <- graph_1 %>%
    mutate(shading = ifelse(sd == 0.2 | sd == 0.6, "shaded", "none"))

  # Custom labels for the rho parameter
  rho_labels <- c('0.4' = "Autocorrelation = 0.4", '0.8' = "Autocorrelation = 0.8")
# make 0.8 before 0.5 in the rho factor
graph_1$rho <- factor(graph_1$rho, levels = c('0.8', '0.4'))
...
graph_1$sd <- factor(graph_1$sd, levels = c(0.2, 0.4, 0.6), labels = c('sd = 0.2', 'sd = 0.4', 'sd = 0.6'))
...
combined_graph_2 <- combined_graph_2 |> 
  mutate(shading = ifelse(rho == 0.8, "shaded", "none"))

combined_graph_2$ci <- factor(combined_graph_2$ci, levels = c('95', '99', '+/- 0.5 BMIz'))

# Create the plot with linetype for ci
combined_graph <- ggplot(combined_graph_2 |> filter(sd != '0.4'), aes(y = bmiz_within_ci, x = as.numeric(n_datapoints), color = forecast_point, linetype = as.factor(sd))) +
  ...
  scale_color_manual(values = c("Max" = "#1A4F66", 
                                ...
                                "Mean + Most Recent" = "#C2B824", 
                                "Most Recent" = "#A481C7")) +  
 scale_linetype_manual(values = c("0.2" = "dashed", "0.6" = "dotted", "Combined" = "solid")) +
  embarktools::embark_theme_a +
  theme(legend.position = 'right') + 
  theme(strip.background = element_rect(fill = strip_fill_light, color = "#F0F0F0"),           
